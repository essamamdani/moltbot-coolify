...
ssh netcup "docker exec <container> ls -la /app/scripts/"
```
...
ssh netcup "docker exec openclaw-qsw0sgsgwcog4wg88g448sgs-103249061467 ls -la /root/openclaw-workspace/skills/"
...
**Current example:**
- Old container: `openclaw-qsw0sgsgwcog4wg88g448sgs-072726042274`
- New container: `openclaw-qsw0sgsgwcog4wg88g448sgs-103249061467`
...
openclaw-qsw0sgsgwcog4wg88g448sgs-072726042274

# New container (running)
openclaw-qsw0sgsgwcog4wg88g448sgs-103249061467

# Skills still there
ssh netcup "docker exec openclaw-qsw0sgsgwcog4wg88g448sgs-103249061467 ls /root/openclaw-workspace/skills/"
# Output: github  session-logs  summarize  weather ✅
```

### ✅ Skills Are Loaded
```bash
ssh netcup "docker exec openclaw-qsw0sgsgwcog4wg88g448sgs-103249061467 openclaw skills list"
...
ssh netcup "docker exec <container> clawhub install trello"
```
...
ssh netcup "docker run --rm -v qsw0sgsgwcog4wg88g448sgs_openclaw-workspace:/data -v /root/backups:/backup alpine tar czf /backup/openclaw-workspace-$(date +%Y%m%d).tar.gz -C /data ."

# Backup config volume
ssh netcup "docker run --rm -v qsw0sgsgwcog4wg88g448sgs_openclaw-config:/data -v /root/backups:/backup alpine tar czf /backup/openclaw-config-$(date +%Y%m%d).tar.gz -C /data ."
```
...
```bash
# Restore workspace
ssh netcup "docker run --rm -v qsw0sgsgwcog4wg88g448sgs_openclaw-workspace:/data -v /root/backups:/backup alpine tar xzf /backup/openclaw-workspace-20260204.tar.gz -C /data"

# Restore config
ssh netcup "docker run --rm -v qsw0sgsgwcog4wg88g448sgs_openclaw-config:/data -v /root/backups:/backup alpine tar xzf /backup/openclaw-config-20260204.tar.gz -C /data"
```
...
```bash
# DON'T DO THIS
ssh netcup "docker exec <container> nano /app/scripts/bootstrap.sh"
```
...
```bash
# DON'T DO THIS
ssh netcup "docker exec <container> apt install some-package"
```
...
```bash
# DON'T DO THIS
ssh netcup "docker exec <container> cp important-file.txt /tmp/"
```
...
```bash
# DON'T DO THIS
ssh netcup "docker volume rm qsw0sgsgwcog4wg88g448sgs_openclaw-workspace"
```
...
**Container:** openclaw-qsw0sgsgwcog4wg88g448sgs-103249061467  
